...
class CSVSpec extends Specification {
  ...
  "CSV" should {
    ...
    "Read from String" in {
      val resource = new String("AAA,BBB,CCC,\"100,000\",\"テスト\"" + util.Properties.lineSeparator + "DDD,EEE,FFF,\"G\nG\",\"HHH\"")
      val actual = CSV.from(resource).asList
      actual must have size 2
      actual(0) must contain(exactly("AAA", "BBB", "CCC", "100,000", "テスト"))
    ...
    "Read from String" in {
      val resource = new String("head1,head2,\"head,3\"" + util.Properties.lineSeparator + "AAA,BBB,CCC" + util.Properties.lineSeparator + "DDD,\"E,E,E\",\"F\nF\"")
